Description
-----------
Pydiction is a filetype plugin for Python files that allows you to Tab-complete Python code; including Python's keywords, standard library and even third-party libraries.  
...
It consists of three main files:
    ...
    pydiction.py -- This is a Python script that was used to generate complete-dict. You can optionally run this script to add more modules to complete-dict.


Install details
---------------
Unix/Linux: Put python_pydiction.vim in ~/.vim/after/ftplugin/   (if that directory doesn't already exist, create it. Vim will know to look there automatically.)
...
Since pydiction uses a dictionary file of possible completion items, it can complete 3rd party modules much more accurately than other ways. You have full control over what it can and cannot complete. If it's unable to complete anything you can either use pydiction.py, to automatically add a new module's contents to the dictionary, or you can manually add them using a text editor. The dictionary is just a plain text file, which also makes it portable across all platforms.  For example, if you're a PyQT user, you can add all the PyQT related modules to the dictionary file (complete-dict) by using pydiction.py.  The default complete-dict already contains most of the standard library, all Python 2.x keywords, Pygame, OpenGL, wxPython, Twisted, ZSI, LDAP, numarray and more.

Also, because pydiction uses a dictionary file, you don't have to import a module before you can complete it, nor do you even have to have the module installed on your machine. This frees you up to use pydiction as a way of looking up what a module attribute is called without having to install it first.

Pydiction is smart enough to know when you're completing a callable method or not and, if you are, it will automatically insert an opening parentheses for you.

The Tab key will work as normal for everything else. Pydiction will only try to use the Tab key to complete Python code if you're editing a Python file and you first type part of some Python code.
...
There is also limited support for string type method completion. For example:
...
make sure you type at least two letters of the method name for this to work.
...
The default complete-dict currently contains:
    ...
    It also contains some popular third-party libraries:
        ...
        pygame, wxPython, twisted, ZSI, numarray, LDAP and OpenGL
...
you can complete dynamic object methods, such as "S.send()", by using Vim 7's system ftplugin "pythoncomplete.vim" by doing:
